...
if __name__ == "__main__":
    ...
    parser.add_argument('--remove-light-pollution-output', type=str, help='Output image name after light pollution removal')
    ...
    parser.add_argument('--remove-light-pollution', help='Estimate and remove light pollution', action='store_true')
    ...
    parser.set_defaults(root_power=0.6, sky_level_factor=0.6, hist_smoothing=601, sky_removal_passes=2, radius=5, sigma=1, remove_light_pollution=False)
    ...
    if args.remove_light_pollution:
        logging.info(f"Light pollution removal")
        img = removeLightPollution(img)
        if args.remove_light_pollution_output:
            logging.info(f"Saving light pollution removal file {args.remove_light_pollution_output}")
            skimage.io.imsave(args.remove_light_pollution_output, (img * np.iinfo(np.uint16).max).astype(np.uint16), plugin='tifffile')
